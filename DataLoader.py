...
def add_default_sub_dim_nat_data(subDimNatDataFile, standardsFile):
	...
	for row in reader:
		if row[0] != "PROMOTION" and row[0] != "PROMODETAIL":
...
def main():
	...
	# If user doesn't create new product key, ask which geographical
	# region the client belongs to (currently only EU or US).
	if productKeyBranch == "N":
	...
	if (subDimNatStandardsFile_New == ""):
		add_default_sub_dim_nat_data(gkaFiles[3],subDimNatStandardsFile)
	else: 
